...
DEFAULT_TIMEOUT = 300
USER_AGENT = "dsh-x-skill/1.0 (+https://github.com/loadchange/dsh-x-search)"
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if data.get("original") and (args.ask or args.command == "ask") and not quiet:
        print("\n--- Original (images read on the server) ---")
        print(data["original"])
    if not quiet:
        lim = data.get("limiter") or {}
...
examples:
  ...
  dsh_x.py check
  dsh_x.py ask https://x.com/someone/status/123 what does this post mean?   # server reads the images, answers directly
